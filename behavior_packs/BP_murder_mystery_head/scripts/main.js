...
function showSetHeadUI(player, showDebugInfo) {
    const mapButtons = Object.keys(info.headData).map(mapName => ({
        type: "button",
        text: mapName,
...
function setHead(mapName, showDebugInfo, showPlayer) {
    ...
    async function tryPlace(mapHeadData) {
        let index = 0;
        for (let head of mapHeadData) {
            ...
            if (mapName === "archivesTopFloor")
                location = lib.Vector3Utils.add(location, 646, -1, 495); // 档案馆顶层比较特殊，需要做坐标变换
            ...
            try {
                ...
                if (error instanceof Error)
                    sendMessage(`§c在放置位于${locationStr}的头颅${id}时遇到了错误，请将下面的错误原因汇报给开发者：\n${error.message}`);
            }
            // 移除常加载区域
