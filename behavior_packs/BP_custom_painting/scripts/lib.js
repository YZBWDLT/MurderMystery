...
class GameSystem {
    ...
    subscribeTimeline(id, callback, interval = 1) {
        ...
        if (this.showDebugMessage)
            minecraft.world.sendMessage(`§a+ 时间线 ${id}`);
        return true;
    }
    /** 取消订阅特定 ID 的时间线。
     * @returns 返回是否成功取消订阅时间线。
     */
    unsubscribeTimeline(id) {
        // 检查时间线是否存在，若不存在则终止运行
        ...
        if (this.showDebugMessage)
            minecraft.world.sendMessage(`§c- 时间线 ${id}`);
        return true;
    }
    /** 取消多个时间线。 */
    unsubscribeTimelines(...ids) {
     ...
     * @returns 返回是否成功订阅延迟。
     */
    subscribeDelay(id, callback, tickDelay = 1) {
        // 检查延迟是否重叠，若存在重叠则阻止运行
        if (this.getAllDelayIds().includes(id))
            return false;
        ...
        if (this.showDebugMessage)
            minecraft.world.sendMessage(`§a+ 延迟 ${id}`);
        return true;
    }
    /** 取消订阅特定 ID 的延迟。
    ...
    unsubscribeDelay(id) {
        ...
        if (this.showDebugMessage)
            minecraft.world.sendMessage(`§c- 延迟 ${id}`);
        return true;
    }
    /** 获取所有延迟的 ID。 */
    getAllDelayIds() {
     ...
     * @returns 返回是否成功添加事件。
     */
    subscribeEvent(id, event, callback, options) {
        // 检查事件是否重叠，若存在重叠则阻止运行
        if (this.getAllEventIds().includes(id))
            return false;
        // 添加事件，并记录到事件列表中，同时追踪该事件
        /** 实际在subscribe函数内执行的回调函数 */
        const subscribeCallback = (event) => {
        ...
        if (this.showDebugMessage)
            minecraft.world.sendMessage(`§a+ 事件 ${id}`);
        return true;
    }
    /** 取消订阅游戏事件、
    ...
    unsubscribeEvent(id) {
        ...
        if (this.showDebugMessage)
            minecraft.world.sendMessage(`§c- 事件 ${id}`);
        return true;
    }
    /** 取消多个事件。 */
    unsubscribeEvents(...ids) {
...
export class StructureUtils {
    /** 放置结构（支持异步）。 */
    static placeAsync(structure, location, options, dimension) {
    ...
    /** 创建特定 ID 的结构。 */
    static add(structureId, dimension, from, to, options) {
...
export class TickingAreaUtils {
     ...
     */
    static add(id, from, to, dimension) {
        try {
...
class ScoreboardObjectiveUtils {
     ...
     */
    static display(displaySlot, id, order = minecraft.ObjectiveSortOrder.Descending) {
     ...
     */
    static addThenDisplay(id, displaySlot, displayName, order = minecraft.ObjectiveSortOrder.Descending) {
...
class ScoreboardPlayerUtils {
    ...
    static getObjective(participantName) {
        return ScoreboardObjectiveUtils.getAll().filter(obj => obj.getScores().some(info => info.participant.displayName === participantName));
    }
    /** 获取分数，若获取不到则设置为默认值。
     * @returns 如果指定的记分项不存在，则返回 undefined。
     */
...
export class DimensionUtils {
     ...
     */
    static getDefault(dimension) {
        const overworld = this.get("overworld");
        if (!dimension)
            return overworld;
     ...
     */
    static divideVolume(volume, maxCapacity = 32768) {
        // 如果区域容量小于最大容量，直接返回该区域
        const volumeCapacity = volume.getCapacity();
        if (volumeCapacity <= maxCapacity)
            return [volume];
        // 【整体思路】
        ...
        /** 最长轴 */
        const longestAxis = Object.keys(axisInfo).sort((axis1, axis2) => axisInfo[axis2].length - axisInfo[axis1].length)[0];
        ...
        for (let i = 0; i < subVolumeAmount; i++) {
            ...
            const thisVolumeFrom = { ...volume.from };
            thisVolumeFrom[longestAxis] = thisVolumeLongestAxisMin;
            const thisVolumeTo = { ...volume.to };
    ...
    /** 镂空一个大区域。 */
    static hollowVolume(volume) {
        const { x: minX, y: minY, z: minZ } = volume.getMin();
        const { x: maxX, y: maxY, z: maxZ } = volume.getMax();
        const hollowedVolume = new minecraft.BlockVolume({ x: minX + 1, y: minY + 1, z: minZ + 1 }, { x: maxX - 1, y: maxY - 1, z: maxZ - 1 });
        const borderVolume = [
    ...
    /** 获取一个区域内的所有位置。 */
    static getLocationsFromVolume(volume) {
...
export class BlockUtils {
     ...
     */
    static get(location, dimension) {
        return DimensionUtils.getDefault(dimension).getBlock(location);
    }
    /** 在两个坐标间填充方块。 */
    static fill(blockData, options, dimension) {
        const volumes = [];
        const { from, to, id, states } = blockData;
        DimensionUtils.divideVolume(new minecraft.BlockVolume(from, to)).forEach(volume => volumes.push(DimensionUtils.getDefault(dimension).fillBlocks(volume, id, options)));
        if (states) {
            volumes
    ...
    /** 在两个坐标间以镂空的形式填充方块。 */
    static fillHollow(blockData, options, dimension) {
        const { from, to, id, states } = blockData;
     ...
     */
    static set(blockData, dimension) {
    ...
    static match(blockData, dimension) {
        ...
        if (states) {
            const hasUnmatchedState = Object.entries(states).some(([state, value]) => {
                ...
                if (block.permutation.getState(state) !== value)
                    return true;
                return false;
            });
            if (hasUnmatchedState)
                return false;
        }
        return true;
    }
...
export class Vector3Utils {
    /** 返回复制后的坐标。 */
    static copy(location) {
        return { x: location.x, y: location.y, z: location.z };
    }
    /** 将某个轴的坐标添加某个特定的值。 */
    static add(location, xAdder = 0, yAdder = 0, zAdder = 0) {
        return { x: location.x + xAdder, y: location.y + yAdder, z: location.z + zAdder };
    }
    /** 返回将输入坐标中心化的坐标。 */
    static center(location) {
     ...
     * @param squared 是否返回模长的平方值以避免运算根号。| 默认值：`false`
     */
    static magnitude(location, squared = false) {
    ...
    /** 返回北方的向量。 */
    static north(location, length = 1) {
        return this.add(location, 0, 0, -length);
    }
    /** 返回南方的向量。 */
    static south(location, length = 1) {
        return this.add(location, 0, 0, length);
    }
    /** 返回西方的向量。 */
    static west(location, length = 1) {
        return this.add(location, -length, 0, 0);
    }
    /** 返回东方的向量。 */
    static east(location, length = 1) {
        return this.add(location, length, 0, 0);
    }
    /** 返回上方的向量。 */
    static up(location, length = 1) {
        return this.add(location, 0, length, 0);
    }
    /** 返回下方的向量。 */
    static down(location, length = 1) {
        return this.add(location, 0, -length, 0);
    }
    /** 返回两向量的点乘（内积）。 */
    static dotProduct(location1, location2) {
        const { x: x1, y: y1, z: z1 } = location1;
        const { x: x2, y: y2, z: z2 } = location2;
        return x1 * x2 + y1 * y2 + z1 * z2;
    }
    /** 返回两向量的叉乘（向量积）。 */
    static crossProduct(location1, location2) {
        const { x: x1, y: y1, z: z1 } = location1;
        const { x: x2, y: y2, z: z2 } = location2;
        return { x: y1 * z2 - y2 * z1, y: z1 * x2 - z2 * x1, z: x1 * y2 - x2 * y1 };
    }
    /** 返回两向量所成的角度（rad） */
    static angle(location1, location2) {
     ...
     */
    static getClosest(location, locationArray) {
     ...
     */
    static getVolumeSector(location, volume) {
        ...
        //    ↓ Z
        if (location.x > xMax)
            return { direction: minecraft.Direction.East, distance: location.x - xMax };
        if (location.x < xMin)
            return { direction: minecraft.Direction.West, distance: xMin - location.x };
        if (location.z > zMax)
            return { direction: minecraft.Direction.South, distance: location.z - zMax };
        if (location.z < zMin)
            return { direction: minecraft.Direction.North, distance: zMin - location.z };
        if (location.y > yMax)
            return { direction: minecraft.Direction.Up, distance: location.y - yMax };
        if (location.y < yMin)
            return { direction: minecraft.Direction.Down, distance: yMin - location.y };
        return { distance: 0 };
    }
    /** 将向量等比例延长。 */
...
export class EntityUtils {
     ...
     */
    static isNearby(entity, location, distance, dimension) {
     ...
     * @param dimension 如果不指定，则默认在实体所在的维度执行。
     */
    static isInVolume(entity, volume, dimension) {
        const executeDimension = dimension ? DimensionUtils.getDefault(dimension) : entity.dimension;
        return executeDimension
     ...
     */
    static removeAll(options, dimension) {
...
export class PlayerUtils {
    ...
    /** 对特定玩家发送通知。 */
    static notify(player, options) {
        const { message, title, subtitle, titleOptions = { fadeInDuration: 10, stayDuration: 70, fadeOutDuration: 20 }, sound, soundOptions, soundDelay, } = options;
        if (message)
            player.sendMessage(message);
        if (title)
            player.onScreenDisplay.setTitle(title, { ...titleOptions, subtitle });
        if (sound) {
            if (soundDelay)
                minecraft.system.runTimeout(() => player.playSound(sound, soundOptions), soundDelay);
            else
...
class InventoryUtils {
    ...
    static isItem(entity, slot, options) {
     ...
     */
    static add(entity, itemId, options, playSound = "player", spawnItemWhenOverflow = true) {
        // 如果该实体不存在物品栏，终止运行
        const inventory = this.get(entity);
        if (!inventory)
            return;
     ...
     */
    static set(entity, slot, itemId, options) {
        const inventory = this.get(entity);
        if (!inventory)
            return;
     ...
     * @returns 返回已清除的物品数量。
     */
    static remove(entity, slot, amount) {
        // 如果实体不存在物品栏，直接终止程序
        const inventory = this.get(entity);
        if (!inventory)
            return 0;
     ...
     * @returns 返回已添加的物品数量。
     */
    static addSlot(entity, slot, amount, itemId, options) {
        // 如果实体不存在物品栏，直接终止程序
        const inventory = this.get(entity);
        if (!inventory)
            return 0;
     ...
     */
    static getAmount(entity, options) {
        return JSUtils.number.sum(this.getValidItems(entity)
            .filter(itemData => ItemUtils.match(itemData.item, options))
            .map(itemData => itemData.item.amount));
    }
    /** 获取特定 ID 的物品数目。 */
    static getTypeAmount(entity, id) {
        return JSUtils.number.sum(this.getValidItems(entity)
...
class EquipmentUtils {
    ...
    /** 获取实体装备栏内对应槽位的物品。 */
    static getItem(entity, equipmentSlot) {
    ...
    static isItem(entity, equipmentSlot, options) {
...
export class ItemUtils {
    ...
    // ===== 通用 =====
    /** 按照给定条件生成 ItemStacks。 */
    static generate(itemId, options = {}) {
        // 解构参数
        ...
        while (amount > 0) {
            ...
            if (enchantments)
                enchantments.forEach(enchantment => this.addEnchantment(itemStack, enchantment));
            if (itemLock)
                itemStack.lockMode = itemLock;
            if (lore)
            ...
            if (unbreakable) {
                ...
                if (durabilityComponent)
                    durabilityComponent.unbreakable = unbreakable;
            }
     ...
     * @remarks 该方法不支持在受限执行下调用。
     */
    static match(itemStack, options) {
        // 获取选项信息和物品相关信息
        const { includeTypeId: optionsTypeId, name: optionsName, itemLock: optionsItemLock, keepOnDeath: optionsKeepOnDeath, lore: optionsLore, canDestroy: optionsCanDestroy, canPlaceOn: optionsCanPlaceOn, unbreakable: optionsUnbreakable, } = options;
        const optionsAmountRange = (() => {
            if (typeof options.amount === "number")
                return [options.amount, options.amount];
            return options.amount;
        })();
        const { typeId: itemTypeId, amount: itemAmount, nameTag: itemName, lockMode: itemItemLock, keepOnDeath: itemKeepOnDeath, } = itemStack;
        ...
        if (optionsTypeId && !optionsTypeId.includes(itemTypeId))
            return false;
        if (optionsAmountRange && (optionsAmountRange[0] > itemAmount || optionsAmountRange[1] < itemAmount))
            return false;
        if (optionsName && optionsName !== itemName)
            return false;
        if (optionsItemLock && optionsItemLock !== itemItemLock)
            return false;
        if (optionsKeepOnDeath && optionsKeepOnDeath !== itemKeepOnDeath)
            return false;
        if (optionsLore && !JSUtils.array.isSameRestrict(optionsLore, itemLore))
            return false;
        if (optionsCanDestroy && !JSUtils.array.isSame(optionsCanDestroy, itemCanDestroy))
            return false;
        if (optionsCanPlaceOn && !JSUtils.array.isSame(optionsCanPlaceOn, itemCanPlaceOn))
            return false;
        if (optionsUnbreakable && optionsUnbreakable !== itemUnbreakable)
            return false;
        // 其余情况均认为可筛选
        return true;
    }
     ...
     */
    static hasItem(entity, matchOptions, searchOptions = {}) {
        ...
        if (cursorInventory && entity.typeId === "minecraft:player") {
            const cursorItem = CursorInventoryUtils.get(entity)?.item;
            if (cursorItem && this.match(cursorItem, matchOptions))
                return true;
        }
        // 检查装备栏的物品
    ...
    /** 获取实体是否拥有物品（命令写法）。 */
    static hasItemCommand(entity, itemId, quantity, location, slot, data) {
        ...
        // 检测物品
        return (entity.runCommand(`execute if entity @s[hasitem={item=${itemId}${quantityStr}${locationStr}${slotStr}${dataStr} }]`)
            .successCount !== 0);
    }
    /** 清除物品（命令写法）。 */
    static removeItem(player, itemId = "", data = -1, maxCount = -1) {
     ...
     */
    static addEntity(location, itemId, options = {}, clearVelocity = false, dimension) {
    ...
    /** 获取物品掉落物。 */
    static getEntity(itemId, dimension) {
        const allItems = DimensionUtils.getDefault(dimension).getEntities({ type: "minecraft:item" });
        if (itemId)
            return allItems.filter(item => item.getComponent("item")?.itemStack.typeId === itemId);
        return allItems;
    }
    /** 清除物品掉落物。 */
    static removeEntity(itemId, dimension) {
    ...
    static addEnchantment(item, enchantment) {
        ...
        if (!comp)
            return item;
        // 如果附魔不能添加，终止运行
        const enchantmentData = { type: new minecraft.EnchantmentType(id), level: level };
        if (!comp.canAddEnchantment(enchantmentData))
            return item;
        // 添加附魔
...
export class UIUtils {
    ...
    static handleNewFormData(thisFormData, showPlayer, parentForm, newFormData) {
        // 如果没有指定新界面，则直接终止
        if (!newFormData)
            return;
    ...
    static createAction(showPlayer, formData) {
        // 表单创建
        ...
        // 显示设置
        // 筛选出所有的 button 组件
        const buttons = formData.components
        ...
        form.show(showPlayer)
            ...
            const newFormData = (() => {
                ...
                if (selection !== undefined)
                    return buttons[selection]?.onClick();
                // 取消表单后
                else if (canceled && formData.onCancel)
                    return formData.onCancel(cancelationReason, formData);
            })();
    ...
    static createMessage(showPlayer, formData) {
        // 表单创建
    ...
    static createModal(showPlayer, formData) {
        // 表单创建
        const form = new ui.ModalFormData();
        if (formData.title)
            form.title(formData.title);
        if (formData.submitButton)
            form.submitButton(formData.submitButton);
        if (formData.components)
            formData.components.forEach(component => {
                if (component.visible === false)
                    return;
        ...
        form.show(showPlayer)
            ...
            if (formValues !== undefined) {
                ...
                formData.components
                    ...
                    if (["dropdown", "toggle", "textField", "slider"].includes(component.type)) {
                        valueComponents.push(component);
                ...
                valueComponents.forEach((component, index) => {
                    switch (component.type) {
                        case "dropdown":
                            component.onSubmit(values[index], component.items);
                            break;
                        case "slider":
                            component.onSubmit(values[index]);
                            break;
                        case "textField":
                            component.onSubmit(values[index]);
                            break;
            ...
            const newFormData = (() => {
                // 提交后
                if (formValues !== undefined && formData.onSubmit)
                    return formData.onSubmit(formValues, formData);
                // 取消表单后
                else if (canceled && formData.onCancel)
                    return formData.onCancel(cancelationReason, formData);
            })();
    ...
    static createAutomatically(showPlayer, formData) {
...
class ArrayUtils {
    ...
    static isSameRestrict(array1, array2) {
    ...
    /** 判断一个数组有多少个相同的数字 `num`。 */
    static countSameNumbers(numberList, num) {
        let map = new Map();
     ...
     */
    static randomWeightedIndex(items) {
        if (items.length === 0)
            return -1;
        const totalWeight = items.reduce((sum, item) => sum + item.weight, 0);
        let random = Math.random() * totalWeight;
...
class TimeDisplayUtils {
    ...
    static showSecondsByTick(tick) {
        const seconds = tick / 20;
        return seconds.toFixed(2);
    }
    /** 根据游戏刻显示多少分钟多少秒。例：1230 → 1:01.50。 */
    static showMinuteAndSecondsByTick(tick) {
    ...
    static showMinuteAndSecondsBySecond(second) {
        const sign = second < 0 ? "-" : "";
    ...
    static formatDateToYYMMDD(date) {
...
export class Debug {
    ...
    /** 获取对象数据。 */
    static getObjectData(object, hasFunction = true) {
        /** 待打印的字符串数组 */ let printString = [];
        // 设置打印的字符串
        if (object == undefined) {
