...
class MurderMysteryEventManager {
    ...
    /** 对玩家设置事件冷却状态。 */
    setEventCooldown(type: string, duration: number) {
        // 设置冷却
        this.eventCooldown[type] = duration;

        // 注册每秒 -1 的冷却时间线
        lib.gameSystem.subscribeTimeline(
            `system${type}EventCooldown`,
            () => {
                const currentDuration = this.eventCooldown[type] ?? 0;
                if (currentDuration <= 1) delete this.eventCooldown[type];
                else this.eventCooldown[type] = currentDuration - 1;
            },
            20,
        );
    }
     ...
     */
    getEventCooldownCountdown(type: string, itemName?: string | "general", notifyPlayer?: minecraft.Entity): number {
...
class MurderMysteryComponents {
    ...
    static interaction(system: MurderMysterySystem) {
        ...
        lib.gameSystem.subscribeEvent("interaction", minecraft.world.beforeEvents.playerInteractWithBlock, event => {
            ...
            if (!isFirstEvent) event.cancel = true;
            /** 交互的默认黑名单。 */
            const banList = [
            ...
            if (banList.includes(block.typeId) && player.getGameMode() !== minecraft.GameMode.Creative) event.cancel = true;
            ...
            const matchedInteraction = comp.find(data => {
                if (lib.Vector3Utils.hasPosition(data.at, block.location)) return true;
                return false;
            });
    ...
    static settings(system: MurderMysterySystem) {
        lib.gameSystem.subscribeEvent("settings", minecraft.world.afterEvents.itemUse, event => {
            if (event.itemStack.typeId === "murder_mystery:settings") MurderMysterySettings.showMainSettingsUI(system, event.source);
        });
    }
    ...
    static playerKillTest(system: MurderMysterySystem) {
        ...
        lib.gameSystem.subscribeEvent("playerKillTestHit", minecraft.world.afterEvents.entityHitEntity, event => {
            ...
            if (attackerMainhandItem?.typeId !== swordId) return;
            // 记录击杀
    ...
    static murdererKnife(system: MurderMysterySystem) {
        lib.gameSystem.subscribeEvent("murdererKnifeTest", minecraft.world.afterEvents.itemUse, event => {
            ...
            if (ironSword.typeId !== swordId) return;
            system.getPlayer(murderer)?.throwingKnife();
        });
        lib.gameSystem.subscribeTimeline(
            "murdererHoldKnifeTest",
            () => {
                system.livingPlayers.murderer.forEach(murderer => {
                    // 如果不是手持剑，则终止运行
                    if (lib.ItemUtils.equipment.getItem(murderer.player, minecraft.EquipmentSlot.Mainhand)?.typeId !== swordId) return;
...
export class MurderMysteryPlayer {
    ...
    throwingKnife() {
        ...
        lib.gameSystem.subscribeEvent(`${murderer.id}UseItemAgain`, minecraft.world.afterEvents.itemUse, event => {
            if (event.itemStack.typeId !== swordId) return;
    ...
    showSwordParticle() {
        const player = this.player;
        const rightDirection = lib.Vector3Utils.normalize(
            lib.Vector3Utils.crossProduct(player.getViewDirection(), { x: 0, y: 1, z: 0 }),
        );
        const location: minecraft.Vector3 = {
    ...
    setEventCooldown(type: string, duration: number) {
        ...
        // 注册每秒 -1 的冷却时间线
        lib.gameSystem.subscribeTimeline(
            `${this.player.id}${type}EventCooldown`,
            () => {
                const currentDuration = this.eventCooldown[type] ?? 0;
                if (currentDuration <= 1) delete this.eventCooldown[type];
                else this.eventCooldown[type] = currentDuration - 1;
    ...
    /** 返回玩家是否处于某个事件的冷却状态中。 */
    getEventCooldownCountdown(type: string, itemName?: string): number {
        const countdown = this.eventCooldown[type] ?? 0;
