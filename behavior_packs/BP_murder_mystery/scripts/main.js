...
class MurderMysteryEventManager {
    ...
    triggerEvent(id, playerData) {
        ...
        if (condition) {
            ...
            const hasBlockUnmatched = isBlock?.some(data => !lib.BlockUtils.match(data)) ?? false;
            if (hasBlockUnmatched)
                return false;
            // 检查玩家高度条件是否通过
            if (playerBelowHeight) {
                if (!playerData)
                    return false;
                if (playerData.player.location.y >= playerBelowHeight)
                    return false;
            }
            // 检查玩家特定冷却是否仍在运行
    ...
    getMysteryPotion(getMysteryPotionEvent, playerData) {
        ...
        if (!playerContainer?.getItem(5))
            replaceSlot = 5;
        else if (!playerContainer?.getItem(7))
            replaceSlot = 7;
        else
            replaceSlot = 0;
...
class MurderMysteryComponents {
    ...
    static getSpecialItem(system) {
        ...
        lib.gameSystem.subscribeTimeline("getSpecialItem", () => {
            const getSpecialItemTimeLeft = system.settings.gaming.getSpecialItemDelay - (system.settings.gaming.timePerGame - system.timeLeft);
            // 对玩家提示
...
class MurderMysteryPlayer {
     ...
     */
    setDead(deathType = gameData.MurderMysteryDeathType.Other, killer) {
        // 如果游戏已结束，直接终止
        if (this.system.gameStage !== GameStage.GamingStage)
            return false;
        // 若该玩家已死亡，则跳过之
