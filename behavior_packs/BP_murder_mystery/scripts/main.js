...
export class MurderMysterySystem {
    ...
    static setEntityState(entity, state, value) {
        entity.setDynamicProperty(state, value);
    }
    /** 获取世界的状态。状态列表可在 {@link WorldDynamicProperties} 检查或注册。 */
    static getWorldState(state, defaultValue) {
        const property = minecraft.world.getDynamicProperty(state);
        return property ?? defaultValue;
    }
    /** 设置世界的状态。状态列表可在 {@link WorldDynamicProperties} 检查或注册。 */
    static setWorldState(state, value) {
...
class MurderMysteryEventManager {
    ...
    getMysteryPotionData(playerData, potionIndex) {
     ...
     * @returns 返回是否成功获得了药水。
     */
    getMysteryPotion(animationLocation, playerData) {
        // ===== 条件检查 =====
        // 如果玩家不是 Player，终止运行
    ...
    drinkMysteryPotion(playerData, potionId) {
        ...
        if (this.inPotionEffect[player.id]) {
            lib.ItemUtils.equipment.set(player, potionId, minecraft.EquipmentSlot.Mainhand, this.getMysteryPotionData(playerData, potionIndex));
     ...
     */
    intoHauntedHouseDoor(playerData, doorLocation, lavaCaveGlassLocation, voidGlassLocation, voidBarrierLocation) {
        // ===== 变量准备 =====
        const player = playerData.player;
        // ===== 条件判断 =====
        // 如果没有方块，则终止运行
        ...
        return new Promise(resolve => {
            lib.gameSystem.subscribeTimeline(`${player.id}IntoHauntedHouseDoor`, time => {
                // 播放音效
                if (time >= 1 && time <= 5)
                    lib.PlayerUtils.notify(player, { sound: "note.bass" });
                // 第 6 秒时随机一个结果
                if (time === 6) {
     ...
     */
    rideMinecart(playerData, consumeGold = 1, from, to, initVelocity) {
        // ===== 变量准备&条件检查 =====
        // 如果不是玩家，阻止触发此事件
        const player = playerData.player;
        if (!isPlayer(player))
...
class MurderMysterySettings {
    ...
    /** 对玩家显示临终遗言 UI。 */
    static showLastWordsUI(system, player) {
        const allLastWords = Object.keys(gameData.lastWords);
        const lastWordButtons = allLastWords.map(lastWord => ({
            type: "button",
    ...
    /** 对玩家显示临终遗言确认 UI。 */
    static showLastWordsConfirmUI(system, player, lastWord) {
        ...
        lib.gameSystem.subscribeEvent("onPlayerHurt", minecraft.world.beforeEvents.entityHurt, event => {
            ...
            minecraft.system.run(() => {
                ...
                if (thisCause === minecraft.EntityDamageCause.lava)
                    playerData.setDead(gameData.MurderMysteryDeathType.Lava);
                // 如果受到溺水伤害，立刻处死
                if (thisCause === minecraft.EntityDamageCause.drowning)
                    playerData.setDead(gameData.MurderMysteryDeathType.Drowned);
            });
            // ===== 触发系统事件 =====
...
export class MurderMysteryPlayer {
    ...
    setDead(deathType = gameData.MurderMysteryDeathType.Other, killer, killDistance = 0) {
        ...
        this.system.livingPlayers.allPlayers.forEach(playerData => {
            ...
            if (playerData.player.id === player.id)
                return;
            // 对其他玩家播放受伤音效
    ...
    haveEnoughGold(count, notify = true) {
     ...
     */
    consumeGold(count) {
        if (!this.haveEnoughGold(count))
            return false;
        if (isPlayer(this.player))
            lib.ItemUtils.removeItem(this.player, goldId, -1, count);
        return true;
    }
    // #endregion
    // #region - 设置物品
