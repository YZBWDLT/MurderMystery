...
**开发者备注**：_指南针目前仍然存在一些问题，原定预计为平民指出弓的位置，然而实测发现指南针指向的是世界出生点的位置，而不是玩家重生点的位置。因此，指南针只能用于杀手的定位。后续的版本中，我们会弃用指南针作为定位系统，而改用原版的定位栏解决问题，这需要 26.30 的脚本系统，所以从下个版本起，将全面提高游戏需求版本到 26.30_
...
- 现在英雄只能是平民了
- 现在在开始游戏前，不再试图清除画  
  **开发者备注**：_然而，目前我们仍然没有添加对阻止玩家破坏画的支持，因为基岩版引擎几乎不允许我们这么做。我们后续可能会使用一些绕弯子的方法阻止类似问题的发生，但不会是现在_
- 现在熔岩不再能造成任何伤害
